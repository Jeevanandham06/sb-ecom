...
public class ProductServiceImpl implements  ProductService {
   ...
   private CartService cartService;
    ...
    public ProductDTO updateProduct(Long productId, ProductDTO productDTO) {
        ...
        cartDTOS.forEach(cart ->cartService.upadateProductInCarts(cart.getCartId(),productId));
        return modelMapper.map(savedProduct, ProductDTO.class);
    }
    ...
    public ProductDTO updateProductImage(long productId, MultipartFile image) throws IOException {
        //Steps to Upload Image to the Product
        // Step 1 : Get Product from Db
        Product productFormDb = productRepository.findById(productId)
                .orElseThrow(() -> new ResourceNotFoundException("Product", "productId", productId));
        // Step 2 : Upload Image to Server & Get the file name Upload Image
        String fileName = fileService.uploadImage(path, image);

        // Step 3 : Update the file name of the product
